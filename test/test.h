...
#define ASSERT_T( expr ) if (!(expr)) { __pf__.nFailures++; fprintf(stderr, "FAILURE: %s: %s line %d\n    " #expr "\n", __FILE__, __FUNCTION__, __LINE__); exit(1); } else { __pf__.nPasses++; }

#define EXPECT_F( expr ) if (expr) { __pf__.nFailures++; fprintf(stderr, "FAILURE: %s: %s line %d\n    " #expr "\n", __FILE__, __FUNCTION__, __LINE__); } else { __pf__.nPasses++; }

#define ASSERT_F( expr ) if (expr) { __pf__.nFailures++; fprintf(stderr, "FAILURE: %s: %s line %d\n    " #expr "\n", __FILE__, __FUNCTION__, __LINE__); exit(1); } else { __pf__.nPasses++; }
